...
> 框架概念统一放在 `docs/technical-solutions/java-concepts.md`。
...
**技术方案文档化（强制）：**
- 敏感数据加密/脱敏的设计方案须写入技术文档 `docs/technical-solutions/sensitive-data-guide.md`
...
- 详细教程见 `docs/technical-solutions/flyway-guide.md`
...
每个功能分支在创建 PR 前，**必须更新 `docs/DEVELOPMENT-PROGRESS.md`**：

- 在「变更日志」顶部新增版本条目（新增/优化/修复分组）
- 更新「已完成功能」列表（勾选已完成的项）
- 更新「待优化项」（标记已完成、新增发现的问题）
- 更新「项目概览」指标（文件数、代码行数等）
- 更新「下一步计划」（基于当前状态调整）

> 这确保进度文档始终与 main 分支同步，避免分支间冲突。
