...
### v0.5.1 — 管理后台导航优化（2026-06-13）

**优化**
- Header 管理后台子菜单数据驱动重构：6 个菜单项抽成 `adminMenuItems` 数组 + `v-for` 渲染，降低与路由表不同步风险
- 父级「管理后台」标题补充 `Tools` 图标，与子项视觉一致
- 新增 `isAdmin` getter（`stores/user.ts`），收敛散落在 Header 的 `role === 'ADMIN'` 字面量判断；路由守卫保留通用 `role !== requiresRole` 不动
